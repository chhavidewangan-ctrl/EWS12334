                ...
                <tr key={p._id}>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 500 }}>{p.employee?.user?.firstName} {p.employee?.user?.lastName}</div>
          ...
          <div className="modal modal-md" style={{ maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div className="modal-body" style={{ overflowY: 'auto', flex: 1, paddingRight: 8 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                ...
                <span className={`tag tag-${statusColor[showPayslip.status]}`}>{showPayslip.status}</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  <h4 style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: 'var(--success)' }}>EARNINGS</h4>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:14, fontWeight:700, marginTop:8, padding:'8px 0', borderTop:'2px solid var(--border)', color:'var(--success)' }}>
                    <span>Gross</span><span>{fmt(showPayslip.totalEarnings)}</span>
                  </div>
                </div>
                <div>
                  <h4 style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: 'var(--danger)' }}>DEDUCTIONS</h4>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:14, fontWeight:700, marginTop:8, padding:'8px 0', borderTop:'2px solid var(--border)', color:'var(--danger)' }}>
                    <span>Total</span><span>-{fmt(showPayslip.totalDeductions)}</span>
                  </div>
                </div>
              </div>
              <div style={{ marginTop:20, padding:16, background:'linear-gradient(135deg,rgba(99,102,241,0.1),rgba(14,165,233,0.1))', borderRadius:'var(--radius-sm)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <span style={{ fontSize:16, fontWeight:600 }}>Net Salary</span>
                <span style={{ fontSize:24, fontWeight:800, color:'var(--primary)' }}>{fmt(showPayslip.netSalary)}</span>
              </div>
              <div style={{ marginTop:16, fontSize:12, color:'var(--text-muted)' }}>
                Attendance: {showPayslip.attendanceSummary?.presentDays} present / {showPayslip.attendanceSummary?.absentDays} absent / {showPayslip.attendanceSummary?.totalDays} total days
              </div>
            </div>
